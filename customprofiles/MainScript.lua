...
local LoadUrl = function(url)
    if url ~= nil and typeof(url) == "string" then
        if string.find(url, "https://") then
            ...
            return loadstring(game:HttpGetAsync("https://raw.githubusercontent.com/Toon-arch/ImpactClientForRoblox/main/" .. url))()
        end
    end
end

local _OriginalDefaultWatermark = "<font color='rgb(47, 172, 255)'>Impact</font> 4.9"
local _DefaultWatermark = _OriginalDefaultWatermark
local _ProtectionService = LoadUrl("modules/protection.impact")
...
local GUIData = (function()
	...
	pcall(function() _ParentClientInterface(screenGui) end)
	...
	UpdateClientSettings = function(savesname)
		...
		local JSONName = ("impact/profiles/" .. tostring(savesname) .. ".JSON")
    ...
    function gui:onRightClick(guiObject, callback)
		guiObject.InputBegan:Connect(function(input)
	...
	function gui:createList(guiObject, guiData)
        ...
        local onDropDownClick = function()
		...
		if guiObject:FindFirstChild("Dropdown") then
			...
			guiObject.Dropdown.MouseButton1Down:Connect(onDropDownClick)
            gui:onRightClick(guiObject.Label, onDropDownClick)
		else
			gui:onDoubleTap(guiObject, function()
				guiData.Open = not guiData.Open
			end)
            gui:onRightClick(guiObject, function()
                guiData.Open = not guiData.Open
            end)
		end
	end
	...
	function gui:addHint(guiObject, hintText)
		...
		guiObject.MouseEnter:Connect(function()
			...
			if currentHint == hintKey and not PanicRunning then
...
local _Flight = (function()
	...
	module.flyStart = function(enabled)
		...
		local newPart = function()
			...
			_ProtectionService.ProtectInstance(Part)
			return Part
		end
...
local _CustomSelfDestructSignals = {}
local _SelfDestruct = function()
	...
	if OnGameCloseFunc ~= nil then
        OnGameCloseFunc:Disconnect()
        OnGameCloseFunc = nil
    end
	if OnTeleportFunc ~= nil then
        OnTeleportFunc:Disconnect()
        OnTeleportFunc = nil
    end
	if GuiRender ~= nil then
        GuiRender:Disconnect()
        GuiRender = nil
    end
    if next(_CustomSelfDestructSignals) ~= nil then
        for index, func in pairs(_CustomSelfDestructSignals) do
...
local Render = gui:create("Container", {
	...
	local PanicEmergency = Render.self:create("Execute", {
		Name = "Panic",
		Hint = "Completely uninject Impact",
		Callback = function()
			_SelfDestruct()
		end,
	})--|
...
return {
	...
	["Watermark"] = function(str)
        if str ~= nil and typeof(str) == "string" then
            ...
            _DefaultWatermark = _OriginalDefaultWatermark
        end
    end,
	["SaveFile"] = function(str)
        if str ~= nil and typeof(str) == "string" then
            ...
            SettingsName = tostring(game.PlaceId)
        end
    end,
	["UpdateSettings"] = function()
        UpdateClientSettings(SettingsName)
    end,
    ["AddOnSelfDestruct"] = function(func)
        if func ~= nil and typeof(func) == "function" then
            table.insert(_CustomSelfDestructSignals, func)
        end
    end,
